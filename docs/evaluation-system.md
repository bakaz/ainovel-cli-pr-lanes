...
普通 A/B 仍遵守“一个变量”原则；本轮五臂只是把同一条基线分别与四个预先登记的参考策略配对。`diagnostics/polisher` 是有意绑定的复合臂，`combination` 是交互臂，因此不能从这两臂单独推断某个子组件的因果贡献。每一臂都必须使用独立输出目录、完整记录实际变更，并由同一套事实门禁和盲评协议裁定。

### 2.8 本轮四方参考的五臂验证协议

本节是本轮文风参考上线的实验矩阵；它是可执行方案，不表示五臂已经运行或当前 CLI 已经提供单次五臂编排。若入口一次只接受一个 variant，就按下表逐臂运行，再用同一份报告格式汇总。具体执行顺序、硬门禁、盲评和停止条件见 [文风参考上线方案](style-reference-rollout.md)。

#### 2.8.1 臂定义

| 臂 | 相对 baseline 的唯一新增内容 | 本臂回答的问题 | 不允许的推断 |
|---|---|---|---|
| `baseline` | 当前正式 prompt、规则和项目事实，不加载本轮外部参考策略 | 当前系统在固定条件下的基准表现是什么 | 不能把一次 baseline 好样本当成质量上限 |
| `scene-craft` | 只加入场景构造技法的抽象约束：目标/行动/冲突/转折、身体反应、空间与感官落点、余波 | 场景级结构技法是否改善可感知性和推进 | 不能归因于人物、事件或句子被“换成参考作品风格” |
| `reference-DNA` | 只加入参考语料提炼出的抽象文风画像：视角距离、句群密度、节奏、细节分配、对白潜台词等 | 抽象文风画像是否改善声音辨识度且不提高相似风险 | 不能复制参考文本的表达、专名、人物或情节 |
| `diagnostics/polisher` | 只启用“诊断 → 最小 edit work order → polisher → 同一事实复核”的闭环 | 最小定点修正是否比一次性重写更稳定、可恢复、可控成本 | 不能把诊断臂和 polisher 臂的效果拆开声称；不能让 polisher 改事实 |
| `combination` | 同时启用前三个策略，仍使用同一项目事实和同一协议 | 组合是否有互补收益，是否引入交互回归 | 不能把组合获胜写成每个组件单独都有效 |

四个外部来源分别以 `scene-craft`、`reference-DNA`、`diagnostics`、`polisher` 四个贡献包登记；实际仓库、版本/提交和许可证信息必须写入 run manifest，不在文档中臆造未核实的来源名称。四个贡献包的参考边界和最小 edit work order 见新文档 §6。

#### 2.8.2 固定变量与匹配运行

五臂不是五次随意生成。每个 `case × repeat` 必须使用同一个干净初始快照，并在所有臂中固定下列变量：

| 变量 | 固定规则 |
|---|---|
| 模型 | provider、模型 ID/版本、角色到模型的映射、API 版本和 fallback 策略均固定；发生 fallback 或模型漂移，该 run 标为无效，不静默混入汇总 |
| 采样 | `temperature`、`top_p`、最大输出、reasoning/思考档位、超时和重试策略固定；报告同时记录实际生效值 |
| 规则 | system/runtime 规则、`meta/user_rules.json` 快照、机械 style rules、字数契约、工具 schema 和 case manifest 固定；参考策略不能覆盖项目事实或规则 |
| seed | 第 `rN` 次重复使用匹配 seed，所有臂共用该 seed；provider 不支持 seed 时记录 `unsupported`，仍做独立重复，不宣称确定性复现 |
| 输入与环境 | 题材/ premise、章节契约、起始 foundation、已完成章节、依赖锁定、二进制基线提交（本轮为 `f8af95f5`）和工具目录固定；臂之间不得共享可写输出 |
| 观测 | `diag`、`stylestat`、usage/cost、checkpoint/session trace 和事实快照用同一版本采集；采集缺失不是“通过” |

同一 case 采用连续五章轨迹，覆盖：开篇/建立、日常或关系推进、冲突升级、揭示/高潮转折、收束后的 hook/aftermath。最小矩阵为 **3 个题材或题材簇 × 每条轨迹 5 章 × 每臂 3 次独立重复**；若产品只服务一个题材，改用 3 个不同 premise 并在结论中限定适用范围。这样每臂至少有 9 条连续轨迹、45 个章节输出，且满足 `stylestat` 至少 5 章的跨章统计前提。每条轨迹在所有臂的相同状态窗口注入一次恢复演练，不能只给某一臂做恢复测试。

seed、题材、章节功能和 repeat 必须按键配对；不可挑选某臂的最好一次与另一臂的最差一次比较。完整的最小样本量、通过阈值和停止条件以 [文风参考上线方案](style-reference-rollout.md) 为准；这些阈值是本轮决策政策，不是文学质量的绝对保证。
