...
// 归一化是机械抽取、不需要推理，不传递 thinking 选项。
...
//
// 归一化是机械抽取、不需要推理：不传递任何 thinking 选项，由 provider 按自身默认策略处理。
// max_tokens 为 normalizeMaxTokens（8192），为 JSON 输出保留充足预算。
		...
		default:
			...
			if out, ok := parseNormalizerJSON(raw); ok {
				...
				return rules.Candidate{
					...
					Preferences:       legacy,
					Scope:             normalizedScope(out.Scope),
...
type normalizerOutput struct {
	...
	Preferences normalizedPreferences `json:"preferences"`
	Scope       string                `json:"scope"` // v1 reply compatibility
	Uncertain   json.RawMessage       `json:"uncertain"`
}

// normalizedPreferences 接受 v1 字符串，也接受 v2 四分区对象。
...
func (p *normalizedPreferences) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &legacy); err == nil {
		p.Legacy = strings.TrimSpace(legacy)
		return nil
	}
	var buckets struct {
	...
	p.Default = strings.TrimSpace(buckets.Default)
...
func (p normalizedPreferences) values() (string, map[rules.RuleScope]string) {
	...
	return strings.TrimSpace(p.Legacy), scoped
}

func normalizedScope(raw string) rules.RuleScope {
	if scope, ok := rules.ParseRuleScope(raw); ok {
		return scope
	}
	return rules.ScopeDefault
}
...
preferences:对象,只有 default / architect / writer / editor 四个可选字符串字段。把同一来源中的每条自然语言偏好按用途分别归入：
- default：所有代理都应遵守的作品定位、跨角色共同原则。
- architect：只影响设定、大纲、节奏规划和剧情展开。
- writer：正文笔法、叙述、对话、措辞、句式等写作规则；Editor 会只读审计这些规则。
- editor：只影响审阅尺度和反馈方式。
- 同一来源可同时填写多个分区；一条规则若确实影响所有角色才放 default。不要拆出 POV 等额外字段。没有内容的分区写空字符串。
...
const normalizerRetryHint = "上面的回复无法解析为 JSON。请严格只输出一个 JSON 对象，含 structured / preferences / uncertain 三个字段；preferences 必须是 default/architect/writer/editor 字符串对象。不要任何解释文字或代码围栏。"
