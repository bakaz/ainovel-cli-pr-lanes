...
func (m *Model) syncRuntimePlaceholder() {
	...
	if m.snapshot.AutoResumePending && !m.snapshot.IsRunning && m.snapshot.Phase != "complete" {
		when := "窗口结束后"
		if !m.snapshot.AutoResumeNotBefore.IsZero() {
			when = m.snapshot.AutoResumeNotBefore.Format("15:04") + " 后"
		}
		if m.snapshot.IdleWritingInPeak && m.snapshot.RunOrigin != "idle_scheduler" {
			m.textarea.Placeholder = "高峰自动暂停已生效，将于北京时间 " + when + " 自动恢复；本窗口继续运行可用 /peak-pause skip"
		} else if m.snapshot.IdleWritingInPeak {
			m.textarea.Placeholder = "闲时写作已在高峰暂停，将于北京时间 " + when + " 自动恢复"
		} else {
			m.textarea.Placeholder = "已安排自动恢复：北京时间 " + when
		}
		return
	}
	switch m.snapshot.RuntimeState {
