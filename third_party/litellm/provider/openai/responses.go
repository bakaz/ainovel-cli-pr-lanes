...
type responsesStream struct {
	...
	toolArgumentsSeen map[string]bool
...
func newResponsesStream(resp *http.Response, model string) *responsesStream {
	...
	return &responsesStream{
		...
		toolArgumentsSeen: make(map[string]bool),
...
func (s *responsesStream) events(name string, raw json.RawMessage) ([]litellm.Event, error) {
	...
	case "response.output_item.added":
		...
		if item.Item.Type == "function_call" {
			...
			key := responsesToolStateKey(item.Item.ID, id)
			if key != "" {
				// Responses providers may repeat output_item.added. The first
				// event opens the call; later events must not create another
				// ToolUseStart for the same item.
	...
	case "response.function_call_arguments.delta":
		...
		key := responsesToolStateKey(delta.ItemID, id)
		if key != "" && delta.Delta != "" {
			s.toolArgumentsSeen[key] = true
		}
	...
	case "response.function_call_arguments.done":
		...
		key := responsesToolStateKey(done.ItemID, id)
		started := key != "" && s.toolStarted[key]
		argumentsSeen := key != "" && s.toolArgumentsSeen[key]
		events := make([]litellm.Event, 0, 3)
		if !started && done.Arguments != "" {
			// Some Responses-compatible providers omit all argument delta
			// events and put the complete JSON only in arguments.done.
			events = append(events, litellm.ToolUseStart{
				ID: id, Name: done.Name, OutputIndex: done.OutputIndex, ItemID: done.ItemID,
			})
			if key != "" {
				s.toolStarted[key] = true
			}
		}
		if !argumentsSeen && done.Arguments != "" {
			// If output_item.added already opened the call, only backfill the
			// missing arguments; do not emit a second ToolUseStart.
			events = append(events, litellm.ToolUseDelta{
				ID: id, OutputIndex: done.OutputIndex, ItemID: done.ItemID,
				ArgumentsDelta: []byte(done.Arguments),
			})
			if key != "" {
				s.toolArgumentsSeen[key] = true
			}
		}
		events = append(events, litellm.ToolUseDone{ID: id, OutputIndex: done.OutputIndex, ItemID: done.ItemID})
		return events, nil
...
func responsesToolStateKey(itemID, id string) string {
	if itemID != "" {
		return itemID
	}
	return id
