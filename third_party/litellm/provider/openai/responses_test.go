...
func TestResponsesStreamDoesNotDuplicateToolStartWhenArgumentsOnlyArriveInDone(t *testing.T) {
	stream := newResponsesStream(streamResponse(strings.Join([]string{
		`event: response.output_item.added`,
		`data: {"type":"response.output_item.added","output_index":0,"item":{"id":"fc_1","type":"function_call","name":"novel_context","call_id":"call_1"}}`,
		`event: response.function_call_arguments.done`,
		`data: {"type":"response.function_call_arguments.done","item_id":"fc_1","output_index":0,"name":"novel_context","arguments":"{}"}`,
		`event: response.completed`,
		`data: {"type":"response.completed","response":{"model":"muse-spark-1.2","status":"completed","usage":{}}}`,
		``,
	}, "\n\n")), "muse-spark-1.2")
	defer stream.Close()
	...
	var deltas []litellm.ToolUseDelta
	...
	if len(starts) != 1 || starts[0].ID != "call_1" || starts[0].Name != "novel_context" {
		t.Fatalf("ToolUseStart events = %+v, want exactly one call_1/novel_context", starts)
	}
	if len(deltas) != 1 || deltas[0].ID != "call_1" || string(deltas[0].ArgumentsDelta) != `{}` {
		t.Fatalf("ToolUseDelta events = %+v, want one complete {} delta", deltas)
	}
	if len(dones) != 1 || dones[0].ID != "call_1" {
		t.Fatalf("ToolUseDone events = %+v, want exactly one call_1", dones)
	}

	resp, err := litellm.Collect(&sliceStream{events: events})
	if err != nil {
		t.Fatalf("Collect returned error: %v", err)
	}
	calls := resp.ToolCalls()
	if len(calls) != 1 || calls[0].ID != "call_1" || calls[0].Name != "novel_context" || string(calls[0].Arguments) != `{}` {
		t.Fatalf("tool calls = %+v, want one valid novel_context call", calls)
	}
}
