...
// buildThinkingBody serializes thinking controls for non-reasoning models.
// The endpoint (proxy or backend) decides whether the thinking parameter is
// accepted; passthrough relays forward it, strict ones reject it. The wire
// shape matches the OpenAI-compatible convention used by reasoning backends
// such as the deepseek-v4-flash proxy:
//
//	{"type":"disabled"} or {"type":"enabled","effort":"low"}
func buildThinkingBody(thinking *litellm.Thinking) (any, error) {
	...
	case litellm.ThinkingEnabled:
		...
		if !isOpenAIReasoningEffort(effort) {
			return nil, fmt.Errorf("openai: unsupported thinking effort %q; use low, medium, high, or xhigh", effort)
		}
		return map[string]any{"type": "enabled", "effort": effort}, nil
	default:
		return nil, fmt.Errorf("openai: unsupported thinking mode %d", thinking.Mode)
	}
}
